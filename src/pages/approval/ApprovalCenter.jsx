...
export default function ApprovalCenter({ t, tk }) {
  const [lr, sLr] = useState([]); const [tc, sTc] = useState([]); const [emps, sEmps] = useState({}); const [ld, sLd] = useState(true)
  ...
  const load = useCallback(async () => {
    ...
    sLr(r || []); sTc(tr || []); sEmps(em); sLd(false)
  }, [tk])
  ...
  const pendL = lr.filter((r) => r.status === "申請中"); const pendT = tc.filter((r) => r.status === "申请中")
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: t.tx, margin: "0 0 16px" }}>✅ 承认中心 <span style={{ fontSize: 12, fontWeight: 400, color: t.wn }}>待审批: {pendL.length + pendT.length}件</span></h2>

      {(pendL.length + pendT.length) === 0 && lr.length === 0 && tc.length === 0 && <div style={{ textAlign: "center", padding: 24, color: t.tm, fontSize: 12 }}>暂无申请</div>}

      {pendL.length > 0 && <>
        <h3 style={{ fontSize: 13, fontWeight: 600, color: t.ts, margin: "0 0 8px" }}>🌴 休假申请（待审批）</h3>
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 20 }}>
          {pendL.map((r) => {
            const lt = LEAVE_TYPES.find((l) => l.v === r.leave_type)
            return <Row key={r.id} name={emps[r.employee_id]?.name || "?"} badge={<span style={{ padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 600, color: lt?.c, background: (lt?.bg || "#eee") + "33" }}>{r.leave_type}</span>} date={r.leave_date} detail={r.reason} isPending onOk={() => actL(r.id, "承認")} onNo={() => actL(r.id, "却下")} />
          })}
        </div>
      </>}

      {pendT.length > 0 && <>
        <h3 style={{ fontSize: 13, fontWeight: 600, color: t.ts, margin: "0 0 8px" }}>🚃 交通费申请（待审批）</h3>
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 20 }}>
      ...
      {lr.filter((r) => r.status !== "申請中").length > 0 && <>
        <h3 style={{ fontSize: 13, fontWeight: 600, color: t.ts, margin: "0 0 8px" }}>📋 已处理休假</h3>
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 20 }}>
          {lr.filter((r) => r.status !== "申請中").slice(0, 20).map((r) => {
            const lt = LEAVE_TYPES.find((l) => l.v === r.leave_type)
            return <Row key={r.id} name={emps[r.employee_id]?.name || "?"} badge={<span style={{ padding: "2px 7px", borderRadius: 5, fontSize: 10, fontWeight: 600, color: lt?.c, background: (lt?.bg || "#eee") + "33" }}>{r.leave_type}</span>} date={r.leave_date} detail={r.reason} status={r.status} />
          })}
        </div>
      </>}
    </div>
